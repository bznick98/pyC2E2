...
class Complete_Automata:
    '''
    The Complete Automaton includes all information for Simulate OR Verification.

    For Simulaion   
            - REQUIRED: automata, initial_state, time_bound
            - OPTIONAL: time_step, simulator
    For Verification
            - REQUIRED: automata, initial_set, unsafe_set, time_bound
            - OPTIONAL: time_step, prop(if property is given, Initial/Unsafe Set will be optional)
    

    automata        - A list of automaton (single automaton will be treated as a single-element list)
    initial_set     - For verify/simulate, A RectangleSet() that specifies the initial condition(range) of the automata
    initial_state   - For simualte, A list of float that used as initial state in simulation
    prop            - Property includes information for Initial Set & Unsafe Set & Time Step & Time Bound & K-Value, prop + automaton can do sim & ver
    time_bound      - Time bound for simulation/verification
    time_step       - Time step for simulation/verification, default = 0.01
    refine_strat    - For verify, Refinement Strategy: {DEF_STRAT(default) | USR_STRAT}
    linear_strat    - For verify, if a mode is linear, it can choose to use different bloat method {GSTAR(default) | RECT}
    simulator       - A String that specifies which simulator to use {ODEINT_FIX(default) | ODEINT_ADP | CAPD}
    name            - Complete Automaton Name in string format
    unsafe_check    - If set to False, then simulation won't check intersection with unsafe set
    '''


    # path to the current file(complete.py) 
    cur_path = os.path.dirname(os.path.abspath(__file__)) + '/'
    work_dir = os.path.join(cur_path, "../work-dir/")


    def __init__(self, automata:list=[], time_bound:float=0, initial_state:list=None, initial_mode:str=None, initial_set:RectangleSet=None, unsafe_set:RectangleSet=None,  
                       time_step:float=0.01, prop:Property=None, refine_strat:str=DEF_STRAT, linear_strat:str=GSTAR, simulator:str=ODEINT_FIX, name:str="default_complete_automata", unsafe_check:bool=True):
        """ Parsing Input """
        # Handle Automata
        # check if input Automaton is in the form of [Automaton1, ...]
        if isinstance(automata, Automaton):
            automata = [automata,]
        elif isinstance(automata, list):
            for automaton in automata:
                if not isinstance(automaton, Automaton):
                    raise Exception("[Complete Automaton Error: All input automata should be in Automaton() type]")
        ...
        if prop is not None:
            if isinstance(prop, Property):
                self.property = prop
                self.initial_set = prop.initial_set
                self.unsafe_set = prop.unsafe_set
                self.initial_state = None
            else:
                raise Exception("[Simulator Property Error]: Input property must be type Property()")
        elif initial_set is not None:
            if isinstance(initial_set, RectangleSet):
                self.initial_set = initial_set
                self.initial_state = None
            else:
                raise Exception("[Simulator Initial Set Error]: Input Initial Set must be type RectangleSet()")
        elif (initial_state is not None) and (initial_mode is not None):
            self.initial_state = initial_state
            self.initial_mode = initial_mode
            self.initial_set = RectangleSet()   # will be a empty set for now, initial_state will be converted to initial set before simulate
        else:
            raise Exception("[Initial Condition Error]: One of {Initial Set | Initial State + Mode | Property} must be provided.")

        # Handle Unsafe set
        if unsafe_set is None:
            self.unsafe_set = RectangleSet()
        ...
        # Optional 
        self.refine_strat = refine_strat
        self.linear_strat = linear_strat
        self.simulator = simulator

        self.name = name
        # if set to false, then simulation won't check unsafe set intersection
        self.unsafe_check_bool = unsafe_check


    def __repr__(self):
        DISPLAY_INFO = "=== Complete_Automata() ===\n"
        DISPLAY_INFO += "Automata: {}\n".format(self.automata_names)
        DISPLAY_INFO += "Initial Set (if verify): {}\n".format(self.initial_set.raw_expression)
        DISPLAY_INFO += "Initial State (if simulate): {}\n".format(self.initial_state)
        DISPLAY_INFO += "Time Step: {}\n".format(self.time_step)
        DISPLAY_INFO += "Time Bound: {}\n".format(self.time_horizon)
        return DISPLAY_INFO

    def __call__(self, **kwargs):
        """ update/assign information by calling Simulator object directly """
        for key, value in kwargs.items():
            # parsing input
            if key == 'automata':
                if isinstance(value, Automaton):
                    value = [value, ]
                elif isinstance(value, list):
                    for a in value:
                        if not isinstance(a, Automaton):
                            raise Exception("[Type Error]: Input should be Automaton() / list of Automaton() Type")
                else:
                    raise Exception("[Type Error]: Input is neither Automaton() or list of Automaton()")
            elif key == 'initial_set' or key == 'unsafe_set':
                if not isinstance(value, RectangleSet):
                    raise Exception("[Type Error]: Initial Set has to be RectangleSet Type")
            # time bound/horizon
            elif key in ['time_bound', 'time_horizon']:
                key = 'time_horizon'
            # 'self.key = value'
            setattr(self, key, value)

    def state_to_set(self, initial_state):
        """ Convert Initial State(list of floats)[a,b,c] To Initial Set(with radius = 0) """
        # Get automaton variable str list. After compose, there will be only 1 automaton in the list
        varname_list = self.automata[0].var_names
        raw_expr = ""
        for dim_idx in range(len(initial_state)):
            state = initial_state[dim_idx]
            raw_expr += "{0} >= {1} && {0} <= {1} ".format(varname_list[dim_idx], state)
            if dim_idx != len(initial_state)-1:
                raw_expr += "&&"
        # convert raw expression to rectangle set
        initial_set = RectangleSet(raw_expr)
        return initial_set

    def input_parse(self, automata:list=[], time_bound:float=0, initial_state:list=[], initial_set:RectangleSet=None, unsafe_set:RectangleSet=None,  
                       time_step:float=0.01, prop:Property=None, refine_strat:str=DEF_STRAT, linear_strat:str=GSTAR, simulator:str=ODEINT_FIX, name:str="Default Name"):
        """ Parsing Input """
        # parse if input Automaton is in the form of [Automaton1, ...]
        if isinstance(automata, Automaton):
            automata = [automata,]
        elif isinstance(automata, list):
            for automaton in automata:
        ...
        if initial_set is not None and not isinstance(initial_set, RectangleSet):
            raise Exception("[Simulator Initial Set Error]: Input Initial Set must be type RectangleSet()")
        # check for unsafe set
        if unsafe_set is not None and not isinstance(unsafe_set, RectangleSet):
        ...
        # For verify, populate Inv Guards
        self.guardResets = None
        self.invariants = None
        ...
        # TODO: If both property & set are given, check if they are equal (or just overwrite property with initial/unsafe set)
 
    def simulate(self):
        # If it's blackbox model, use TC_Simulate instead
        # TODO: Is Blackbox model support multi-agent?
        if len(self.automata) == 1 and self.automata[0].isBlackBox:
            blackbox = self.automata[0]
            trace = blackbox.TC_Simulate(self.initial_mode, self.initial_state, self.time_horizon)
            return trace
        
        # If it's Automaton with explicit dynamics, proceed normally
        # Compose all
        self.compose_all()
        ...
        # Load Config 
        with open(self.cur_path + '../config.json') as f:
            config = json.load(f)
        ...
        trace = FileHandler.read_from_output(self.cur_path + '../work-dir/' + self.name)
...
# ==========================================================================================
    ...
    def generate_simulator(file_path, automata, **kwargs):
        ...
        **kwargs    - step_type and other parameters
        ...
        file_path = Complete_Automata.cur_path + file_path
    ...
    def compose_all(self):
        ...
        while len(automata_list) > 1:
            ...
            automata_list.append(self.compose(automaton1, automaton2))

        # If user provided initial state, after Compose to one Automaton, convert initial state -> initial set:
        if self.initial_state is not None:
            self.initial_set = self.state_to_set(self.initial_state)
            self.initial_set.initial_mode = self.initial_mode
        ...
        writer = open(self.cur_path + "../work-dir/ThinVarProp","w")
        writer.write(thinvarprop)
        writer.close()
        
        writer = open(self.cur_path + "../work-dir/ThinVarList","w")
    ...
    def compose(automaton1, automaton2):
        ...
        for m1 in automaton1.modes:
            for m2 in automaton2.modes:
                ...
                dai_dict = {}
                Complete_Automata.construct_output_dict(cross_mode, m1.dais, dai_dict)
                Complete_Automata.construct_output_dict(cross_mode, m2.dais, dai_dict)
                Complete_Automata.replace_dais(cross_mode, dai_dict)
    ...
    def printHybridSimGuardsInvariants(self):
        self.printGuardsInvariants(self.cur_path + "../work-dir/hybridSimGI.cpp", True)

    def printBloatedSimGuardsInvariants(self):
        self.printGuardsInvariants(self.cur_path + "../work-dir/bloatedSimGI.cpp", False)
    ...
    def initialize_cpp_model(self, config, sim_bool):
        ...
        for m_i, m in enumerate(self.modes):
            fn = self.cur_path + '../work-dir/jacobiannature' + str(m_i+1) + '.txt'
        ...
        model.set_sim_unsafe_check(self.unsafe_check_bool)
        ...
        model.set_visualize_filename(self.work_dir + self.name)
        # TODO: Find a way to Set relative path so that C++ can execute bianry from any directory
        model.set_executable(self.work_dir + config['simulator_name'])
        model.set_work_dir(self.work_dir)

        code_file=open(self.cur_path + "../work-dir/main.cpp","w+")
        ...
        code_file.write("    cpp_Model.setVisualizeFilename(\"" + os.path.join(self.work_dir, self.name) + "\");\n")
        code_file.write("    cpp_Model.setExecutable(\"../work-dir/" + config['simulator_name']+"\");\n")
        code_file.write("    cpp_Model.setWorkDir(\"" + self.work_dir + "\");\n")
    ...
    def compile_executable(self, config):
        ...
        if((self.simulator == ODEINT_ADP) or (self.simulator == ODEINT_FIX)):
            ...
            p = subprocess.Popen(args, cwd= self.cur_path + "../work-dir")
            p.communicate()
        else:
            print("  Using CAPD Simulator.\n")
            command_line = "g++ -w -O2 simulator.cpp -o" + config['simulator_name'] + "`../capd/bin/capd-config --cflags --libs`"
            p = subprocess.Popen(command_line, cwd= self.cur_path + "../work-dir", shell=True)
            p.communicate()
